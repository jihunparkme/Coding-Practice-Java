...
public class BOJ9019 {
	...
	private static String process(int A, int B) {
		...
		while(!q.isEmpty()) {
			...
			if(!checked[res]) {
				checked[res] = true;
				q.add(new Info(res, now.comd + "D"));
			}
			
			// S 는 n에서 1 을 뺀 결과. n이 0 이라면 9999 가 대신 레지스터에 저장된다.
			res = now.n == 0 ? 9999 : now.n - 1;
			if(!checked[res]) {
				checked[res] = true;
				q.add(new Info(res, now.comd + "S"));
			}

			// L 은 n의 각 자릿수를 왼편으로 회전(d2, d3, d4, d1)
			res = (now.n % 1000) * 10 + (now.n / 1000);
			if(!checked[res]) {
				checked[res] = true;
				q.add(new Info(res, now.comd + "L"));
			}
			
			// R 은 n의 각 자릿수를 오른편으로 회전(d4, d1, d2, d3)
			res = (now.n % 10) * 1000 + (now.n / 10);
			if(!checked[res]) {
				checked[res] = true;
				q.add(new Info(res, now.comd + "R"));
			}
		}
